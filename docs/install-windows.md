...
The install may also print a warning that npm skipped install scripts for
`node-pty`. WSL2's Ubuntu is glibc Linux, and on glibc Linux (x64 or arm64)
this needs no action: node-pty's bundled prebuild loads and a PTY spawns fine
with the scripts skipped (verified). If `quadwork start` or `quadwork doctor`
then reports `node-pty is unusable`, run:
```bash
