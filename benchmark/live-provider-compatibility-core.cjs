...
function reviewedContract(value) {
  exact(value, ['adapter', 'executable_digest', 'executable_path', 'identity', 'resolved_path', 'version_digest'], 'live_reviewed_contract');
  ...
  return Object.freeze({ adapter, executable_digest: value.executable_digest, executable_path: value.executable_path, identity: identity(value.identity), resolved_path: value.resolved_path, version_digest: value.version_digest });
}
function validateReviewedIdentity(contract) {
  required(digest(fs.readFileSync(__filename)) === contract.identity.harness_digest, 'live_identity_not_reviewed');
  required(digest(fs.readFileSync(path.join(__dirname, '..', 'server', 'index.js'))) === contract.identity.source_digest, 'live_identity_not_reviewed');
  required(digest(SAFE_WORKLOAD) === contract.identity.workload_digest, 'live_identity_not_reviewed');
}
...
async function runReviewedCompatibility(reviewed, value, runtime) {
  exact(value, ['adapter', 'evidence_directory', 'executable', 'root_directory'], 'live_run_shape'); const contractReview = reviewedContract(reviewed); validateReviewedIdentity(contractReview); const adapter = ADAPTERS[value.adapter]; required(adapter && adapter.id === contractReview.adapter.id, 'live_adapter_not_supported'); const selectedCaps = caps(runtime);
  ...
  const runIdentity = contractReview.identity; const resolvedExecutable = executable(contractReview, value.executable); const compiled = profile(adapter, root); assertHostIsolation(adapter, compiled, root, runtime); const preFacts = rootFacts(root, runtime);
...
module.exports = Object.freeze({ ADAPTERS, CAPS, EVIDENCE_MARKER, LiveCompatibilityError, ROOT_MARKER, createDisposableLiveCompatibilityEvidenceRoot, createDisposableLiveCompatibilityRoot, runReviewedCompatibility, sanitizedEnvironment });
