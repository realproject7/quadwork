...
test('candidate input list is explicit and includes the V2 launch chain, reviewed evidence, sandbox, workload, and runner', () => {
  assert.deepEqual(profiles.CANDIDATE_FILES, [
    'server/index.js', 'server/config.js', 'server/agent-lifecycle.js', 'server/project-lifecycle.js', 'server/pty-dispatcher.js', 'server/resource-runtime-owner.js', 'server/file-chat.js', 'server/reviewed-execution-profiles.js', 'server/reviewed-execution-live-capability.js', 'benchmark/live-provider-reviewed-contracts.cjs', 'benchmark/v2-product-path-core.cjs', 'benchmark/reviewed-execution-contract.cjs', 'benchmark/reviewed-execution-runner.cjs', 'benchmark/reviewed-execution-live-runner.cjs',
  ]);
