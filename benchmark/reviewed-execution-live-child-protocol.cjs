...
function rootFacts(root) {
  ...
  const entries = []; const walk = (dir, relative = '') => { for (const name of fs.readdirSync(dir).sort()) { const file = path.join(dir, name), rel = relative ? `${relative}/${name}` : name, stat = fs.lstatSync(file); if (stat.isSymbolicLink() || (!stat.isFile() && !stat.isDirectory()) || !sameUser(stat)) throw new Error('reviewed_execution_root_unsafe'); entries.push(`${rel}:${stat.isDirectory() ? 'd' : 'f'}:${mode(stat).toString(8)}:${stat.isFile() ? sha256(fs.readFileSync(file)) : '-'}`); if (stat.isDirectory()) walk(file, rel); } }; walk(checked); const digest = sha256(entries.join('\n')); return Object.freeze({ root_digest: digest, entry_digest: digest, entry_count: entries.length });
}
...
async function prepareFixedChild(profile) {
  if (!profile || fixed) throw new Error('reviewed_execution_child_state'); const started = Date.now(); let facts, gate, prepared;
...
module.exports = Object.freeze({ prepareFixedChild, completeFixedChild, failedChildReport, testHooks: Object.freeze({ readGateReceipt, report, rootFacts, sourceFacts }) });
