...
async function completeFixedChild(role, runtime) {
  ...
  try {
    ...
    return report(state.profile, state.facts, { gate_receipt_digest: state.gate.receipt_digest, result_class: decision.result_class, launch_claim_state, failure_stage: launch_claim_state === 'none' ? 'prelaunch' : 'postclaim', launch_diagnostic, output_bytes: observed.output_bytes, output_capped: observed.output_capped, elapsed_ms: Date.now() - state.started, root_cleanup_ok: decision.root_cleanup_ok, survivor_free: decision.survivor_free, rechecked, pre: state.pre, post });
  } finally { fixed = null; }
}
