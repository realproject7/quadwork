...
async function prepareFixedChild(profile) {
  ...
  } catch { const root_cleanup_ok = prepared?.root ? removeOwnedRoot(prepared.root) : false; return report(profile, facts, { gate_receipt_digest: gate?.receipt_digest || null, launch_claim_state: 'none', failure_stage: 'prelaunch', elapsed_ms: Date.now() - started, root_cleanup_ok, survivor_free: true, cleanup_attestation: root_cleanup_ok ? 'none' : 'root' }); }
}
...
async function completeFixedChild(role, runtime) {
  ...
  try {
    ...
    return report(state.profile, state.facts, { gate_receipt_digest: state.gate.receipt_digest, result_class: decision.result_class, launch_claim_state, failure_stage: decision.result_class === 'completed' ? 'none' : 'postclaim', launch_diagnostic, pre_observer_pty_data_seen, pre_workload_output_seen, workload_write_attempted, workload_submitted_at_launch, terminal_exit_phase, lifecycle_verified: decision.lifecycle_verified, sentinel: completion_sentinel, output_bytes: observed.output_bytes, output_capped: observed.output_capped, elapsed_ms: Date.now() - state.started, root_cleanup_ok: decision.root_cleanup_ok, survivor_free: decision.survivor_free, cleanup_attestation: decision.cleanup_attestation, rechecked, pre: state.pre, post });
  } catch {
    launch_claim_state = launchClaimState(state);
    provider_turns = launch_claim_state === 'none' ? 0 : 1;
