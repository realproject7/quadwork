...
const ADAPTERS = Object.freeze({
  codex: Object.freeze({ id: 'codex', role: 'benchmark_codex', model_id: 'gpt-5.6-luna', command: REVIEWED.codex.executable_path }),
  claude: Object.freeze({ id: 'claude', role: 'benchmark_claude', model_id: 'claude-sonnet-4-6', command: REVIEWED.claude.executable_path }),
});
...
function executable(adapter) {
  const reviewed = REVIEWED[adapter.id]; required(reviewed && reviewed.executable_path === adapter.command, 'product_path_adapter_unreviewed');
...
function configFor(adapter, root, port) {
  ...
  return Object.freeze({
    ...
    projects: [{ id: 'benchmark-product-path', name: 'Benchmark product path', idle: true, chat_mode: 'file', repositories: [{ key: 'benchmark', repo: 'local/benchmark', working_dir: repository, primary: true }], agents: { [adapter.role]: { cwd: repository, command: adapter.command, model: adapter.model_id, auto_approve: false, mcp_inject: 'none' } } }],
  });
}
function writeConfig(root, config) {
  required(OWNED_ROOTS.has(root), 'product_path_root_not_owned'); const home = path.join(root, 'home'), directory = path.join(home, '.quadwork'), filename = path.join(directory, 'config.json');
  fs.mkdirSync(directory, { mode: 0o700 }); fs.chmodSync(directory, 0o700); fs.writeFileSync(filename, JSON.stringify(config), { mode: 0o600, flag: 'wx' }); fs.chmodSync(filename, 0o600);
  return Object.freeze({ home, config: filename });
}
function safeEnvironment(home) {
  required(safePath(home), 'product_path_environment');
  const out = { HOME: home, USERPROFILE: home, PATH: process.env.PATH || '', LANG: process.env.LANG || 'C', LC_ALL: process.env.LC_ALL || 'C', TERM: 'xterm-256color', GIT_CONFIG_NOSYSTEM: '1', GIT_CONFIG_GLOBAL: os.devNull, GIT_TERMINAL_PROMPT: '0' };
...
function workerResult(child, timeoutMs) {
  return new Promise(resolve => {
    ...
    const timer = setTimeout(() => { try { child.kill('SIGKILL'); } catch {} finish({ result_class: 'timeout', cleanup_ok: false }); }, timeoutMs); timer.unref?.();
    const observe = chunk => { bytes += Buffer.byteLength(chunk); if (bytes > CAPS.max_output_bytes) { overflow = true; try { child.kill('SIGKILL'); } catch {} return; } buffer += chunk.toString('utf8'); };
...
async function runReviewedProductPath(value) {
  ...
  const root = createRoot({ parent_dir: value.parent_dir }); const evidence = createEvidence(root); const exec = executable(adapter); repositoryFacts(root); const port = await freePort(); const config = configFor(adapter, root, port); const locations = writeConfig(root, config); const started = Date.now();
  const child = spawn(process.execPath, [path.join(__dirname, 'v2-product-path-worker.cjs'), adapter.id], { cwd: path.join(__dirname, '..'), env: { ...safeEnvironment(locations.home), QUADWORK_PRODUCT_PATH_PORT: String(port) }, stdio: ['ignore', 'pipe', 'pipe'], shell: false, windowsHide: true });
  const result = await workerResult(child, CAPS.max_elapsed_ms + 15_000); const report = redactedReport(adapter, exec, { ...result, elapsed_ms: Date.now() - started, config_isolated: fs.existsSync(locations.config) && path.dirname(locations.config) !== path.join(os.homedir(), '.quadwork') }); persist(evidence, report);
  return Object.freeze({ evidence_directory: evidence, report });
}

module.exports = Object.freeze({ ADAPTERS, CAPS, EVIDENCE_MARKER, ProductPathError, ROOT_MARKER, WORKLOAD, createDisposableProductPathRoot: createRoot, noUnsafeArgs, runReviewedProductPath, safeEnvironment, testHooks: Object.freeze({ checkedRoot, configFor, createEvidence, executable, persist, redactedReport, repositoryFacts, workerResult, writeConfig }) });
