...
function fixedFailure(profile, result, provider_turns, candidate_digest = null, worker_report_disposition = 'none') { return Object.freeze({ schema_version: 1, purpose: 'reviewed_v2_product_path_live_attempt', profile_id: profile.id, backend: profile.backend, model: profile.model, expected_head: null, candidate_digest, gate_receipt_digest: null, result_class: result, provider_turns, launch_claim_state: provider_turns === 0 ? 'none' : 'unverified', failure_stage: 'parent_unverified', launch_diagnostic: 'none', pre_observer_pty_data_seen: false, pre_workload_output_seen: false, workload_write_attempted: false, workload_submitted_at_launch: false, terminal_exit_phase: 'none', worker_report_disposition, lifecycle_verified: false, sentinel_digest: null, output_bytes: 0, output_capped: false, elapsed_ms: 0, root_cleanup_ok: false, survivor_free: false, source_rechecked_before_prompt: false, gate_rechecked_before_prompt: false, pre_root_facts: null, post_root_facts: null, credential_copy_or_store_api_used: false, keychain_immutability_claimed: false, peer_level_network_filter_available: false, release_evidence: false }); }
...
function reportShape(value, profile, candidate_digest) {
  ...
  return !!value && typeof value === 'object' && !Array.isArray(value)
    ...
    && (zeroTurnPrelaunch || claimedComplete || claimedPostlaunch || unverifiedPostlaunch) && childFactsConsistent
    ...
    && value.worker_report_disposition === 'none' && typeof value.root_cleanup_ok === 'boolean' && typeof value.survivor_free === 'boolean'
