...
const role = 'benchmark_codex'; const profile = profiles.PROFILES.v2_codex_readonly_v1;
...
let secret; try { secret = Buffer.alloc(32); if (fs.readSync(4, secret, 0, 32, null) !== 32) throw new Error('short'); } catch { process.exit(1); }
...
process.send({ type: 'reviewed_execution_ready', nonce, candidate_digest: candidate, worker_digest: process.env.QUADWORK_REVIEWED_WORKER_DIGEST, proof: crypto.createHmac('sha256', secret).update(nonce).digest('hex') });
process.once('message', message => { if (message?.type !== 'reviewed_execution_admit' || message.nonce !== nonce || !/^[a-f0-9]{64}$/.test(message.admission) || message.proof !== crypto.createHmac('sha256', secret).update(`${nonce}:${message.admission}`).digest('hex')) return process.exit(1); void child.prepareFixedChild(profile).then(report => { if (report) process.send({ type: 'reviewed_execution_result', report }, () => process.exit(0)); else require('../server/index.js'); }).catch(() => process.send({ type: 'reviewed_execution_result', report: child.failedChildReport(role) }, () => process.exit(1))); });
