...
// and it only parses help output in a sandboxed, time-bounded throwaway HOME.
// It never sends a workload or makes a provider request, so its result is not
...
// Help output needs no network and no provider state. The pinned Claude CLI
// prefetches Keychain items at startup through /usr/bin/security, so that
// command is denied. Writes stay in the throwaway HOME, and the user's
// provider-state and Keychain paths cannot be opened.
function sandboxProfile(home, userHome) {
  required(sandboxSafePath(home) && sandboxSafePath(userHome), 'flag_check_isolation_unavailable');
  return [
    '(version 1)',
    '(allow default)',
    '(deny network*)',
    ...
    '(deny file-write*)',
    `(allow file-write* (subpath "${home}") (literal "/dev/null"))`,
    `(deny file-read* file-write* (prefix "${userHome}/.claude") (subpath "${userHome}/.codex") (subpath "${userHome}/Library/Keychains"))`,
  ].join('\n');
}
...
// The fixed PATH makes a bare `security` resolve only to the denied command.
function run(runtime, profileText, command, args, place, maxBytes, timeout) {
  const result = (runtime?.spawn_sync || spawnSync)(SANDBOX_EXECUTABLE, ['-p', profileText, command, ...args], { cwd: place.home, env: { ...core.sanitizedEnvironment(), HOME: place.home, PATH: '/usr/bin:/bin', TMPDIR: place.tmp }, shell: false, stdio: ['ignore', 'pipe', 'pipe'], timeout, killSignal: 'SIGKILL', maxBuffer: maxBytes, windowsHide: true });
  ...
  return Object.freeze({ code: result.status, signal: result.signal, failed: result.error !== undefined, timed_out: result.error?.code === 'ETIMEDOUT', overflow: result.error?.code === 'ENOBUFS', output_digest: digest(Buffer.concat([stdout, stderr])), stdout });
}
...
function checkFlags(contract, homeParent, runtime) {
  const reviewed = core.testHooks.executable(contract, contract.executable_path);
  const parent = checkedParent(homeParent), selected = caps(runtime), startedAt = Date.now(), adapter = contract.adapter;
  const report = fields => Object.freeze({ schema_version: 1, purpose: 'live_provider_flag_check', provider_support_evidence: false, adapter: adapter.id, pinned_version: core.testHooks.pathVersion(contract.resolved_path), executable_digest: reviewed.digest, version_digest_matched: false, help_digest: null, subcommand_accepted: null, positionals_accepted: false, flags: Object.freeze([]), result_class: 'isolation_unavailable', external_process_started: false, elapsed_ms: Date.now() - startedAt, ...fields });
  let userHome; try { userHome = os.userInfo().homedir; } catch { userHome = null; }
  if (!sandboxAvailable() || !sandboxSafePath(userHome)) return report({});
  const place = createHome(parent);
  try {
    const profileText = sandboxProfile(place.home, userHome);
    // A nested or refused sandbox fails here, before any pinned CLI starts.
    const probe = run(runtime, profileText, '/usr/bin/true', [], place, 1024, selected.max_elapsed_ms);
    if (probe.failed || probe.code !== 0 || probe.signal) return report({});
    const version = run(runtime, profileText, reviewed.path, ['--version'], place, selected.max_version_output_bytes, selected.max_elapsed_ms);
    if (!core.testHooks.versionMatches(version, contract)) return report({ result_class: 'version_failed', external_process_started: true });
    const subcommand = adapter.argv[0].startsWith('-') ? null : adapter.argv[0];
    const help = run(runtime, profileText, reviewed.path, subcommand ? [subcommand, '--help'] : ['--help'], place, selected.max_help_bytes, selected.max_elapsed_ms);
    if (help.failed || help.code !== 0 || help.signal) return report({ result_class: 'help_failed', version_digest_matched: true, external_process_started: true });
    ...
    return report({ version_digest_matched: true, help_digest: digest(help.stdout), subcommand_accepted, positionals_accepted, flags: Object.freeze(walked.flags), result_class: accepted ? 'flags_accepted' : 'flag_rejected', external_process_started: true });
  } finally { fs.rmSync(place.home, { recursive: true, force: true }); }
}

function runFlagCheck(value = {}) {
  ...
  return Object.freeze(['codex', 'claude'].map(id => {
    const reviewed = REVIEWED_EXECUTIONS[id]; required(reviewed?.adapter === id && core.ADAPTERS[id], 'live_adapter_not_supported');
    try { return checkFlags({ ...reviewed, adapter: core.ADAPTERS[id] }, value.home_parent); }
    catch (error) {
      if (!(error instanceof LiveCompatibilityError)) throw error;
      return Object.freeze({ schema_version: 1, purpose: 'live_provider_flag_check', provider_support_evidence: false, adapter: id, pinned_version: core.testHooks.pathVersion(reviewed.resolved_path), result_class: 'not_checked', reason: error.message, external_process_started: false });
    }
  }));
}
...
function main(argv) {
  ...
  try {
    ...
    results = runFlagCheck({ home_parent: argv[1] });
  } catch (error) {
...
module.exports = Object.freeze({ CAPS, SANDBOX_EXECUTABLE, runFlagCheck, testHooks: Object.freeze({ checkFlags, helpOptions, sandboxProfile, walk }) });
