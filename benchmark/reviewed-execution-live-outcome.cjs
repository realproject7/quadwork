...
function observe(chunks = []) { let output = '', output_bytes = 0, output_capped = false; for (const chunk of chunks) { output_bytes += Buffer.byteLength(chunk); if (output_bytes > OUTPUT_CAP_BYTES) { output_capped = true; break; } output += String(chunk); } return Object.freeze({ output_bytes, output_capped, sentinel: output.split(/\r?\n/).includes(SENTINEL) }); }
...
module.exports = Object.freeze({ OUTPUT_CAP_BYTES, SENTINEL, WORKLOAD, observe, finalize });
