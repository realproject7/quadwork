...
function workload(value) {
  ...
  const overlapsProtectedPath = (taskPath, protectedPath) => taskPath === protectedPath || taskPath.startsWith(`${protectedPath}/`) || protectedPath.startsWith(`${taskPath}/`);
  if (tasks.some((task) => value.read_only_paths.some((protectedPath) => overlapsProtectedPath(task.path, protectedPath)))) fail("benchmark_workload_task_path_read_only", "workload task path overlaps a read-only input");
