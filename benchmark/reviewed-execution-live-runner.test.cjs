...
const fork = (file, args, options) => {
  ...
  child.send = message => {
    ...
    const oneTurnUnsafe = resultHarnessMode === 'one-turn-unsafe'; const malformedClaim = resultHarnessMode === 'malformed-claim'; const invalidClaimStage = resultHarnessMode === 'invalid-claim-stage'; const zeroTurnCompleted = resultHarnessMode === 'zero-turn-completed'; const claimedNonzeroExit = resultHarnessMode === 'claimed-nonzero-exit';
    const cleanupFailure = resultHarnessMode === 'prelaunch-cleanup-failure' || oneTurnUnsafe;
    const report = { schema_version: 1, purpose: 'reviewed_v2_product_path_live_attempt', profile_id: profile.id, backend: profile.backend, model: profile.model, expected_head: null, candidate_digest: options.env.QUADWORK_REVIEWED_CANDIDATE_DIGEST, gate_receipt_digest: null, result_class: cleanupFailure ? 'cleanup_failed' : malformedClaim || claimedNonzeroExit ? 'attempt_indeterminate' : prelaunch ? 'preflight_blocked' : 'completed', provider_turns: prelaunch || zeroTurnCompleted ? 0 : 1, launch_claim_state: prelaunch || zeroTurnCompleted ? 'none' : malformedClaim || invalidClaimStage ? 'unverified' : 'claimed', failure_stage: invalidClaimStage || prelaunch || zeroTurnCompleted ? 'prelaunch' : cleanupFailure || malformedClaim || claimedNonzeroExit ? 'postclaim' : 'none', lifecycle_verified: !prelaunch && !malformedClaim, sentinel_digest: prelaunch || malformedClaim || claimedNonzeroExit ? null : 'c'.repeat(64), output_bytes: 0, output_capped: false, elapsed_ms: 0, root_cleanup_ok: !cleanupFailure, survivor_free: true, source_rechecked_before_prompt: !prelaunch, gate_rechecked_before_prompt: !prelaunch, pre_root_facts: facts, post_root_facts: facts, credential_copy_or_store_api_used: false, keychain_immutability_claimed: false, peer_level_network_filter_available: false, release_evidence: false };
    if (resultHarnessMode === 'duplicate') {
      ...
      process.nextTick(() => { child.emit('exit', claimedNonzeroExit ? 1 : 0); setImmediate(() => child.emit('message', { type: 'reviewed_execution_result', report })); });
    }
    return true;
  };
  return child;
...
function claimedNonzeroExitHarness() { return resultHarness('claimed-nonzero-exit'); }
...
test('parent preserves an attested claimed failure result after a later non-zero worker exit', async () => {
  const result = await claimedNonzeroExitHarness().run();
