...
const reviewedGateHome = typeof process.env.QUADWORK_REVIEWED_GATE_HOME === 'string' && path.isAbsolute(process.env.QUADWORK_REVIEWED_GATE_HOME) && !/[\0\r\n]/.test(process.env.QUADWORK_REVIEWED_GATE_HOME) ? process.env.QUADWORK_REVIEWED_GATE_HOME : os.homedir();
const GATE_PARENT = path.join(reviewedGateHome, 'Library', 'Application Support', 'QuadWork', 'reviewed-execution-gates');
const LEDGER_PARENT = path.join(reviewedGateHome, 'Library', 'Application Support', 'QuadWork', 'reviewed-execution-ledger-parent');
