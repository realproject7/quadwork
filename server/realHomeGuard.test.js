...
const posix = process.platform !== "win32"; // the stand-in gh is POSIX only
...
const run = spawnSync(process.execPath, [path.join(serverDir, "run-tests.js")], {
  ...
  encoding: "utf8",
  timeout: 30_000,
  env: { ...process.env, HOME: operatorHome, USERPROFILE: operatorHome, QW_GUARD_OPERATOR_CONFIG: operatorConfig },
});
...
if (posix) {
  assert.match(verdict("2-gh")[0], /FAIL \(exit 0; guard: 2 /);
...
console.log("realHomeGuard.test.js: PASS (refused and charged: ambient HOME, operator HOME, gh, late gh, shell-made ~/.quadwork; clean file passes)");
